<?php
/**
 * Template Name: Smart Systems Landing Page
 * Description: Immersive 3D landing page for Smart Systems Implementation
 */
...
<head>
    ...
    <style>
        /* Remove all WordPress theme styling */
        #wpadminbar { display: none !important; }
        html { margin-top: 0 !important; }
        body.admin-bar { margin-top: 0 !important; }
        header, footer, .site-header, .site-footer, nav, .navigation { display: none !important; }
        #page, #content, .site, .site-content, main, article { 
            margin: 0 !important; 
            padding: 0 !important; 
...
<body <?php body_class(); ?>>
    ...
    <script>
        // Remove WordPress theme elements that might interfere
        document.addEventListener('DOMContentLoaded', function() {
            // Hide admin bar
            const adminBar = document.getElementById('wpadminbar');
            if (adminBar) adminBar.style.display = 'none';
            
            // Remove theme wrappers that might constrain the canvas
            const themeElements = ['header', 'footer', 'nav', '.site-header', '.site-footer', '.navigation'];
            themeElements.forEach(selector => {
                document.querySelectorAll(selector).forEach(el => {
                    if (!el.closest('#canvas-container') && !el.closest('.ui-overlay')) {
